#include<stdio.h> void swap(int a, int b); void swap_addr(int * a, int * b); void changeArray(int* ptr);
...
int pointer_main(void) {
	...
	////미션맨 !
	//
	////첫 번째 미션 : 아파트의 각 집에 방문하며 문에 적힌 암호 확인
	//int* 미션맨; //포인터 변수
	//미션맨 = &철수;
	...
	//배열할 때, arr2 -> 주소 int arr2[3] = { 10, 20, 30 }; //changeArray(arr2); changeArray(&arr2[0]); for (int i = 0; i < 3; i++) {
